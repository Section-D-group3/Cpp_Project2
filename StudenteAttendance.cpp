...
void display(){
  ...
  if(readfile.fName_v.size() == 1){
    ...
    if(choice==1){
      ...
      for(int i = 0; i < readfile.fName_v.size(); ++i){
        if(i == 0){
          cout<<readfile.id_v[i]<<"\t\t"<<readfile.fName_v[i]<<"\t\t"<<readfile.lName_v[i]<<"\t\t"<<readfile.gender_v[i]<<endl;
        }else{
          cout<<readfile.id_v[i]<<"\t\t"<<readfile.fName_v[i]<<"\t\t"<<readfile.lName_v[i]<<"\t\t"<<readfile.gender_v[i]<<endl;
        }
        drawTable();
...
void search(){
  ...
  for(int i = 0; i < readfile.id_v.size(); ++i){
    if(i == (readfile.id_v.size()-1) && (readfile.id_v[i] != id)){
      ...
      drawTable();
      cout<<readfile.id_v[0]<<"\t\t"<<readfile.fName_v[0]<<"\t\t"<<readfile.lName_v[0]<<"\t\t"<<readfile.gender_v[0]<<endl;
      drawTable();
      cout<<readfile.id_v[i]<<"\t\t"<<readfile.fName_v[i]<<"\t\t"<<readfile.lName_v[i]<<"\t\t"<<readfile.gender_v[i]<<endl;
      drawTable();
      break;
